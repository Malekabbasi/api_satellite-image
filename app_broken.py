...
try:try:
    ...
    logging.warning("scikit-image non disponible, certaines améliorations seront limitées")scikit-image non disponible, certaines améliorations seront limitées")
...
def get_sentinelhub_config() -> SHConfig:
    """Configure SentinelHub avec les variables d'environnement"""
    config = SHConfig()
...
def s2_to_rgb(image: np.ndarray, gamma: float = 2.2, percentile: float = 99.9) -> np.ndarray:
    """Convertir une image Sentinel-2 en RGB avec correction gamma"""
    rgb = image[:, :, [2, 1, 0]]  # B04, B03, B02
    rgb = np.clip(rgb, 0, 1)
...
def calculate_masked_average(index_array: np.ndarray, mask: np.ndarray) -> float:
    """Calculer la moyenne d'un indice dans une zone masquée"""
    masked_values = index_array[mask]
    return np.nanmean(masked_values) if len(masked_values) > 0 else 0.0
...
def transparent_cmap(cdef transparent_cmap(cmap_name: str, alpha: float = 0.7):
    """Créer une colormap avec transparence"""
    cmap = plt.cm.get_cmap(cmap_name)
    colors = cmap(np.arange(cmap.N))
    colors[:, -1] = alpha
    return mcolors.ListedColormap(colors)ap._init()
    cmap._lut[:, -1] = alpha
    return cmap
...
def create_mask_fdef create_mask_from_geometry(geometry: dict, image_shape: tuple, 
                            bounds: Optional[tuple] = None) -> np.ndarray:e, 
                            bounds: tuple) -> np.ndarray:
    """Créer un masque à partir de la géométrie GeoJSON"""
    try:
        mask = np.ones(image_shape[:2], dtype=bool)
...
def calculate_masked_averadef calculate_masked_average(index_array: np.ndarray, mask: np.ndarray) -> float:index_array: np.ndarray, mask: np.ndarray) -> float:
    """Calculer la moyenne d'un indice dans une zone masquée"""
    masked_values = index_array[mask]
    return np.nanmean(masked_values) if len(masked_values) > 0 else 0.0
...
def calculate_masked_average(index_array: np.ndarray, mask: np.ndarray) -> float:
    """Calculer la moyenne d'un indice dans une zone masquée"""
    masked_values = index_array[mask]
    return np.nanmean(maske    return float(np.nanmean(masked_values)) if len(masked_values) > 0 else 0.0n(masked_values) if len(masked_values) > 0 else 0.0
